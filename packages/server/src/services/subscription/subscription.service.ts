...
export async function createSubscription(
  ...
  // Notify emp-billing via its /empcloud-webhook handler so it can
  // provision the client (idempotent), create the plan, create the
  // recurring subscription, and generate the first invoice. Replaces the
  // old direct-call chain (autoProvisionClient / createBillingPlan /
  // createBillingSubscription) which created duplicate client records
  // that didn't match the records the webhook handler creates -- meaning
  // invoices generated by billing's recurring jobs never showed up under
  // EmpCloud's mapped client id.
  //
  // Trial subscriptions still get notified so the subscription exists in
  // billing with the right metadata; the webhook handler generates a draft
  // invoice that billing's normal jobs will activate when the trial ends.
  // Free-tier (price 0) subscriptions don't need emission at all.
  if (pricePerSeat > 0) {
    billingEmitter
...
export async function updateSubscription(
  ...
  // Notify emp-billing of seat / plan changes (non-blocking). The webhook
  // handler updates the existing subscription row in billing, matched via
  // metadata.empcloud_subscription_id; if no match it logs a warning and
  // returns 200, so this is safe even before the backfill has run.
  billingEmitter
