...
const STATUS_CONFIG: Record<string, { color: string; icon: typeof Clock }> = {
  ...
  under_investigation: { color: "bg-yellow-100 text-yellow-700", icon: Search },
  escalated: { color: "bg-orange-100 text-orange-700", icon: ArrowUpRight },
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t("whistleblowing.track.title")}</h1>
          <p className="text-sm text-gray-500">{t("whistleblowing.track.subtitle")}</p>
        </div>
      </div>

      {/* Search Box */}
      <div className="bg-white rounded-xl shadow-sm border p-6 mb-6">
        <label className="block text-sm font-medium text-gray-700 mb-2">{t("whistleblowing.track.caseNumber")}</label>
      ...
      {isLoading && (
        <div className="bg-white rounded-xl shadow-sm border p-12 text-center">
          ...
          <p className="text-gray-500">{t("whistleblowing.track.lookingUp")}</p>
        </div>
      )}
      ...
      {isError && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
          ...
          <p className="text-red-700 font-medium">{t("whistleblowing.track.notFound")}</p>
        ...
        <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
          <div className="p-6 border-b bg-gray-50">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">{t("whistleblowing.track.caseNumber")}</p>
          ...
          <div className="p-6 space-y-4">
            <div>
              <p className="text-sm text-gray-500">{t("whistleblowing.track.category")}</p>
              <p className="font-medium text-gray-900 capitalize">
                {t(`whistleblowing.reports.category.${data.category}`, { defaultValue: data.category?.replace(/_/g, " ") })}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-500">{t("whistleblowing.track.subject")}</p>
              <p className="font-medium text-gray-900">{data.subject}</p>
            </div>
            <div className="flex gap-6">
              <div>
                <p className="text-sm text-gray-500">{t("whistleblowing.track.submitted")}</p>
                <p className="text-gray-900">{new Date(data.created_at).toLocaleDateString()}</p>
              </div>
              {data.resolved_at && (
                <div>
                  <p className="text-sm text-gray-500">{t("whistleblowing.track.resolved")}</p>
          ...
          {data.updates && data.updates.length > 0 && (
            <div className="p-6 border-t">
              <h3 className="text-sm font-semibold text-gray-700 uppercase mb-4">{t("whistleblowing.track.updates")}</h3>
