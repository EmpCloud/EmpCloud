    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{t("whistleblowingDashboard.header.title")}</h1>
            <p className="text-sm text-gray-500">{t("whistleblowingDashboard.header.subtitle")}</p>
          </div>
        </div>
        <Link
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <Link to="/whistleblowing/reports" className="block text-left w-full bg-white rounded-xl shadow-sm border p-5 transition-all hover:border-brand-300 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-brand-500">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm text-gray-500">{t("whistleblowingDashboard.stats.totalReports")}</span>
          </div>
        ...
        <Link to="/whistleblowing/reports" className="block text-left w-full bg-white rounded-xl shadow-sm border p-5 transition-all hover:border-brand-300 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-brand-500">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm text-gray-500">{t("whistleblowingDashboard.stats.open")}</span>
          </div>
        ...
        <Link to="/whistleblowing/reports" className="block text-left w-full bg-white rounded-xl shadow-sm border p-5 transition-all hover:border-brand-300 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-brand-500">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm text-gray-500">{t("whistleblowingDashboard.stats.resolved")}</span>
          </div>
        ...
        <Link to="/whistleblowing/reports" className="block text-left w-full bg-white rounded-xl shadow-sm border p-5 transition-all hover:border-brand-300 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-brand-500">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm text-gray-500">{t("whistleblowingDashboard.stats.avgResolution")}</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border p-5">
          <h3 className="text-sm font-semibold text-gray-700 uppercase mb-4">{t("whistleblowingDashboard.section.bySeverity")}</h3>
            ...
            {data.by_severity?.map((item: { severity: string; count: number }) => (
              <div key={item.severity} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-700 capitalize">{t(`whistleblowingDashboard.severity.${item.severity}`, { defaultValue: item.severity })}</span>
                </div>
                <span className="text-sm font-semibold text-gray-900">{Number(item.count)}</span>
              </div>
            ))}
            {(!data.by_severity || data.by_severity.length === 0) && (
              <p className="text-sm text-gray-400">{t("whistleblowingDashboard.empty.noData")}</p>
            )}
          </div>
        </div>
        ...
        {/* By Category */}
        <div className="bg-white rounded-xl shadow-sm border p-5">
          <h3 className="text-sm font-semibold text-gray-700 uppercase mb-4">{t("whistleblowingDashboard.section.byCategory")}</h3>
          <div className="space-y-3">
            {data.by_category?.map((item: { category: string; count: number }) => (
              <div key={item.category} className="flex items-center justify-between">
                <span className="text-sm text-gray-700 capitalize">{t(`whistleblowingDashboard.category.${item.category}`, { defaultValue: item.category.replace(/_/g, " ") })}</span>
                <span className="text-sm font-semibold text-gray-900">{Number(item.count)}</span>
              </div>
            ))}
      ...
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
        <div className="p-5 border-b">
          <h3 className="text-sm font-semibold text-gray-700 uppercase">{t("whistleblowingDashboard.section.recentReports")}</h3>
        </div>
            ...
            <tbody className="divide-y">
              {data.recent.map((r: { id: number; case_number: string; category: string; severity: string; subject: string; status: string; created_at: string }) => (
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3 text-sm text-gray-700 capitalize">{t(`whistleblowingDashboard.category.${r.category}`, { defaultValue: r.category.replace(/_/g, " ") })}</td>
                  <td className="px-5 py-3">
